...
void main() {
    ...
    albums.get(0).addToPlayList("Speed king", playList);  // Does not exist
    albums.get(0).addToPlayList(9, playList);
    System.out.println((albums.get(0)).albumn.size());
    albums.get(1).addToPlayList(3, playList);
    albums.get(1).addToPlayList(2, playList);
    albums.get(1).addToPlayList(24, playList);  // There is no track 24
}
...
private ArrayList<Song> albumn=new ArrayList<>();
    ...
    public void addSong(String title,double duration){
        albumn.add(new Song(title,duration));
    }

    public void addToPlayList(String name,LinkedList<Song>playlist){

        for (Song song:albumn){

            if(song.title.equals(name)){
                System.out.println("""
                        track exits adding the song to the playlist!
                        """);
           ...
           return;
                ...
                System.out.println("Track doesnt found!");
    ...
    public void addToPlayList(int trackNumber,LinkedList<Song>playlist){
        ...
        if( albumn.size()>=trackNumber &&albumn.get(trackNumber-1)!=null ){
            System.out.println("track found adding the sont to the playlist");
        playlist.add(albumn.get(trackNumber-1));
        }
        else{
            System.out.println("track number entered not found!");
        }
